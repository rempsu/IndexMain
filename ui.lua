...
local function buildUI(parent, settings, toggleKey)
    ...
    local function section(title, order)
        ...
        spad.PaddingTop    = UDim.new(0, 6)   -- tight: header close to items
        spad.PaddingBottom = UDim.new(0, 0)   -- no trailing gap before next section
        spad.Parent = s

        local slayout = Instance.new("UIListLayout")
        slayout.SortOrder = Enum.SortOrder.LayoutOrder
        slayout.Padding = UDim.new(0, 0)      -- no gap between rows
        slayout.Parent = s
        ...
        slabel.Size = UDim2.new(1, 0, 0, 12)  -- was 16
    ...
    local function toggleRow(parent, text, key, order)
        ...
        row.Size = UDim2.new(1, 0, 0, 26)     -- was 36
        row.BackgroundTransparency = 1
        row.BorderSizePixel = 0
        row.LayoutOrder = order + 1            -- offset past label (0) and div (1)
        row.Parent = parent
    ...
    key_row.Size = UDim2.new(1, 0, 0, 26)     -- was 36
    key_row.BackgroundTransparency = 1
    key_row.LayoutOrder = 2                    -- after label (0) and div (1)
